...
export default {
  data() {
    return {
      ...
      examinationNotFoundError: false,
  ...
  computed: {
    isDataValid() {
      ...
      const validExaminationId = /^[1-9]\d*$/.test(this.examinationId);
  ...
  methods: {
    ...
    async getExamination() {
      try {
        // const { data: examinationData } = await fetch(
        //   `http://localhost/api/examination/check/${examinationId.value}`,
        //   {}
        // );

        const response = await fetch(
          `http://localhost/api/examination/check/${this.examinationId}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const examinationData = await response.json();
    ...
    getLoanCalculate() {
      ...
      if (loanCalc < 5001) {
        calcInterest = 0.1;
      } else if (loanCalc < 15001) {
        calcInterest = 0.5;
      } else {
        calcInterest = 0.75;
      }

      this.loanCalculate = loanCalc;
      this.interest = calcInterest;

      console.log("loanCalculate:");
      console.log(this.loanCalculate);
    ...
    async checkData() {
      ...
      try {
        const { data: examination } = await useMyFetch(
          `examination/check/${this.examinationId}`,
          {}
        );
        ...
        if (examination.value.id != null) {
          ...
          console.log("Examination found:", this.examination);
        } else {
    ...
    storeData() {
      ...
      localStorage.setItem("examinationId", this.examinationId);
      localStorage.setItem("loanCalculate", this.loanCalculate);
      localStorage.setItem("examination", JSON.stringify(this.examination));
